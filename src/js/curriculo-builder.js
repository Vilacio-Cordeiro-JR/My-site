...
function initBuilder() {
	...
	byId("print-cv").addEventListener("click", () => {
		// ativa modo currículo
		document.body.classList.add("curriculo-page");

		const cv = document.querySelector(".cv-page");
		if (cv) {
			cv.classList.add("curriculo-container");
		}

		// força reflow (evita bug de render antes do print)
		setTimeout(() => {
			window.print();

			// volta ao modo normal depois de imprimir
			document.body.classList.remove("curriculo-page");
			if (cv) {
				cv.classList.remove("curriculo-container");
			}
		}, 50);
	});
...
window.builderRenderAll = renderAll;
